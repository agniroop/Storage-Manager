<?php
	...
	if(!isset($_GET['search'])){
		$result=mysqli_query($link,"SELECT * FROM indent WHERE status=1 AND passed_by_id='".$_SESSION['user_name']."'");
	} elseif($_GET['search']=="date"){
		$result=mysqli_query($link,"SELECT * FROM indent WHERE status=1 AND passed_by_id='".$_SESSION['user_name']."' ORDER BY ".$_GET['search']." DESC");
	} elseif ($_GET['search']=="department") {
		# code...
		$result=mysqli_query($link,"SELECT * FROM indent WHERE status=1 AND passed_by_id='".$_SESSION['user_name']."' ORDER BY ".$_GET['search']);
	}
	?>
	<!DOCTYPE html>
	<html>
	...
	<?php while($row=mysqli_fetch_assoc($result)){
		...
		?>
				<tr>
					...
					<td><a href="indent_details.php?id=<?php echo $row['id'] ?>" class="btn btn-primary btn-block">Details</a></td>
				</tr>
	<?php } ?>
	</tbody>
	</table>
	...
	<script type="text/javascript">
		function check(){
			...
			if (val==0) {
				window.location="approve_indent.php";
			} else if(val==1){
				window.location="approve_indent.php?search=date";
			} else if(val==2){
				window.location="approve_indent.php?search=department";
			}
		}
	</script>
	</body>
